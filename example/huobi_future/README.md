...
本文主要介绍如何通过框架SDK开发 [Huobi Future](https://www.hbdm.com/zh-cn/contract/exchange/) 交易所的交易系统。
...
- 注册 [Huobi Future](https://www.hbdm.com/zh-cn/contract/exchange/)  的账户，并且创建 `ACCESS KEY` 和 `SECRET KEY`，AK有操作委托单权限；


### 2. 一个简单的策略

> 我们选用的交易对(合约)为 `ETH190906`， 策略执行的几个步骤:  
...
- ACCOUNTS 指定需要使用的交易账户，注意platform是 `huobi_future`；
- strategy 策略的名称；
- symbol 策略运行的交易对(合约)，比如: `ETH190906` ；
- contract_code 合约代码，比如: `ETH190906`；
- contract_type 合约类型，比如：`next_week`；
